...
- (id)initWithFrame:(CGRect)frame menus:(NSArray *)aMenusArray
{
    ...
    ENDRADIUS =110.0f;
    FARRADIUS =130.0f;
    STARTPOINT =CGPointMake(960, 700);
    TIMEOFFSET = 0.026f;
    ANGLE = M_PI_2;
...
-(void)setCenterButtonVisible:(BOOL)visible{
    _addButton.hidden = !visible;
}
-(void)createSubItems{
    _menuItems = [NSMutableArray arrayWithCapacity:_menuItems.count];
    if (self) {
        ...
        for (int i = 0; i < count; i ++)
        {
            ...
            item.endPoint = CGPointMake(STARTPOINT.x - ENDRADIUS * sinf(i * ANGLE / (count - 1)), STARTPOINT.y - ENDRADIUS * cosf(i * ANGLE / (count - 1)));
            item.nearPoint = CGPointMake(STARTPOINT.x - NEARRADIUS * sinf(i * ANGLE / (count - 1)), STARTPOINT.y - NEARRADIUS * cosf(i * ANGLE / (count - 1)));
            item.farPoint = CGPointMake(STARTPOINT.x - FARRADIUS * sinf(i * ANGLE / (count - 1)), STARTPOINT.y - FARRADIUS * cosf(i * ANGLE / (count - 1)));
