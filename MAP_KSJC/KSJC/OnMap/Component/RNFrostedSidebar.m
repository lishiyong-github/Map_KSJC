...
- (instancetype)init {
    if (self = [super init]) {
        ...
        _label = [[UILabel alloc] initWithFrame:CGRectMake(12, 50, 50, 20)];
...
- (instancetype)initWithImages:(NSArray *)images selectedIndices:(NSIndexSet *)selectedIndices borderColors:(NSArray *)colors labels:(NSArray *)labels{
    if (self = [super init]) {
        ...
        _borderWidth = 2;
        _itemBackgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.25];
        ...
        [_images enumerateObjectsUsingBlock:^(UIImage *image, NSUInteger idx, BOOL *stop) {
            ...
            if (nil!=labels &&idx<labels.count) {
                view.label.text = [labels objectAtIndex:idx];
            }
...
- (instancetype)initWithImages:(NSArray *)images selectedIndices:(NSIndexSet *)selectedIndices borderColors:(NSArray *)colors {
    return [self initWithImages:images selectedIndices:selectedIndices borderColors:colors labels:nil];
}
...
- (void)showInViewController:(UIViewController *)controller animated:(BOOL)animated {
    ...
    //blurImage = [blurImage applyBlurWithRadius:5 tintColor:self.tintColor saturationDeltaFactor:1.8 maskImage:nil];
    UIImage *blurImage = [UIImage imageNamed:@"sildebar-background.jpg"];
    [self rn_addToParentViewController:controller callingAppearanceMethods:YES];
    self.view.frame = CGRectMake(0, 0, 200, controller.view.bounds.size.height);
...
- (void)handleTap:(UITapGestureRecognizer *)recognizer {
    ...
    if (! CGRectContainsPoint(self.contentView.frame, location)) {
        [self dismissAnimated:YES];
        [self.delegate sidebar:self willDismissFromTap:YES];
    }
    else {
